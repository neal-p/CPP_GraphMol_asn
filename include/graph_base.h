...
class GraphBase {
 public:
  ...
  int getNVertices() { return n_vertices_; }

  int getNEdges() { return n_edges_; }
  ...
  std::pair<int, int> getEdge(int e) { return edges_[e]; }
  ...
  T getVertexAttribute(const std::string& name, int v) const {
    ...
    //    3. converting that from an Attribute pointer to an AttributeValue<T>
    //    pointer with dynamic cast
    ...
    while (!Q.empty()) {
      ...
      for (auto v : getNeighbors(u)) {
        ...
        if (curr_color == White) {
          ...
          setVertexAttribute("_distance", v,
                             getVertexAttribute<int>("_distance", u) + 1);
  ...
  std::vector<std::vector<int>> getDistanceMatrix() {
    ...
    for (int v = 0; v < n_vertices_; v++) {
      BFS(v);  // sets the distance to the query vertex in the "distance" vertex
               // attribute
